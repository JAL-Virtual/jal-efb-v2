  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g., 240"
            />
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">0° = North, 90° = East, 180° = South, 270° = West</p>
          </div>
          ...
          <div>
            <label className="block text-sm font-medium mb-2">Runway Number</label>
            <input
              type="number"
              min="1"
              max="36"
              ...
              placeholder="e.g., 24"
            />
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Runway 24 = 240° heading</p>
          </div>
          
          <div>
            <label className="block text-sm font-medium mb-2">Crosswind Limit (kts)</label>
            <input
              type="number"
              min="0"
        ...
        {/* Results Section */}
        <div className="space-y-4">
          <h3 className="text-lg font-semibold mb-4">Wind Components</h3>
          ...
          <div className="grid grid-cols-1 gap-4">
            <div className={`p-4 rounded-lg ${windCalc.headwind > 0 ? 'bg-green-100 dark:bg-green-900/30' : windCalc.headwind < 0 ? 'bg-red-100 dark:bg-red-900/30' : 'bg-gray-100 dark:bg-gray-800'}`}>
              ...
              <div className="text-sm opacity-70">kts</div>
            </div>
          ...
          {windCalc.crosswind > windCalc.crosswindLimit && (
            <div className="p-4 rounded-lg bg-red-100 dark:bg-red-900/30 border border-red-300 dark:border-red-700">
              <div className="flex items-center gap-2 text-red-800 dark:text-red-200">
                ...
                <span className="font-semibold">Crosswind Limit Exceeded!</span>
              </div>
              <p className="text-sm text-red-700 dark:text-red-300 mt-1">
                Crosswind ({windCalc.crosswind} kts) exceeds limit ({windCalc.crosswindLimit} kts)
              </p>
            </div>
          )}
          ...
          {windCalc.crosswind > windCalc.crosswindLimit * 0.8 && windCalc.crosswind <= windCalc.crosswindLimit && (
            <div className="p-4 rounded-lg bg-yellow-100 dark:bg-yellow-900/30 border border-yellow-300 dark:border-yellow-700">
              <div className="flex items-center gap-2 text-yellow-800 dark:text-yellow-200">
                ...
                <span className="font-semibold">Approaching Crosswind Limit</span>
              </div>
              <p className="text-sm text-yellow-700 dark:text-yellow-300 mt-1">
                Crosswind ({windCalc.crosswind} kts) is close to limit ({windCalc.crosswindLimit} kts)
              </p>
            </div>
          )}
          ...
          {windCalc.tailwind > 0 && (
            <div className="p-4 rounded-lg bg-orange-100 dark:bg-orange-900/30 border border-orange-300 dark:border-orange-700">
              <div className="flex items-center gap-2 text-orange-800 dark:text-orange-200">
                ...
                <span className="font-semibold">Tailwind Present</span>
              </div>
              <p className="text-sm text-orange-700 dark:text-orange-300 mt-1">
                Consider runway performance with {windCalc.tailwind} kts tailwind
              </p>
            </div>
          )}
        </div>
