  ...
  return (
    ...
    >
      ...
      <div style={{ width: "100vw", height: "100vh", zIndex: 1 }}>
        <MapContainer
          ref={mapRef as any}            // <-- grab the Leaflet map instance
          center={[35, 137]}
          zoom={5}
          zoomControl={false}
          ...
          // use whenReady with zero-arg signature; access map via ref
          whenReady={() => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span title={getCallsign(flight)} style={{ overflow: "hidden", textOverflow: "ellipsis" }}>
          {getCallsign(flight)}
        </span>
      </div>
      ...
      <div style={{ padding: 14 }}>
        <div style={{ marginBottom: 8 }}>
          <Badge>👨‍✈️ {flight?.user?.name || "—"}</Badge>
        </div>
        ...
        {(flight.status_text || flight.status) && (
          <div style={{ textAlign: "center", marginTop: 6 }}>
            <Badge>{flight.status_text || flight.status}</Badge>
          </div>
        )}
      </div>
    </div>
